...
def init_db():
    ...
    for col in ['college', 'phone', 'github_profile', 'avatar']:
        try:
...
def register():
    if request.method == 'POST':
        ...
        if avatar and avatar.filename and allowed_file(avatar.filename):
            avatar_filename = secure_filename(f"avatar_{email}_{avatar.filename}")
            avatar.save(os.path.join(AVATAR_FOLDER, avatar_filename))

        conn = get_db_connection()
        try:
            conn.execute(
                "INSERT INTO users (name, email, password, role, college, phone, github_profile, avatar) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
def edit_profile():
    ...
    if request.method == 'POST':
        ...
        avatar = request.files.get('avatar')
        if avatar and avatar.filename and allowed_file(avatar.filename):
            avatar_filename = secure_filename(f"avatar_{session['email']}_{avatar.filename}")
            avatar.save(os.path.join(AVATAR_FOLDER, avatar_filename))

        conn.execute('''
            UPDATE users SET name=?, college=?, phone=?, github_profile=?, avatar=?
            WHERE id=?
        ''', (name, college, phone, github_profile, avatar_filename, session['user_id']))
        conn.commit()
...
@app.route('/api/my_submission')
def my_submission_api():
    ...
    return jsonify({'total': 0, 'score': None, 'submit_time': None})
...
def review_submission(submission_id):
    ...
    if request.method == 'POST':
        ...
        conn.execute('''
            UPDATE submissions SET admin_review = ?, score = ? WHERE id = ?
